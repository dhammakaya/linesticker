...
//var s_Word = '1738223';
var s_Word = '8467632';
var sUrl = "https://store.line.me/stickershop/product/"+s_Word;
...
rp(options)
	.then((data) => {
		var SourceHtml1 = JSON.stringify(data);
		//console.log(JSON.stringify(data));
		getCompletedAndPushToArray(SourceHtml1);
	})
	.catch((err) => {
		console.log(err);
	});

function getCompletedAndPushToArray(stickerData) {
    ...
    if (SourceHtml1 == "") {
	 ...
	 //console.log(oSourceHtml);
	 updateFlie_HTML(oSourceHtml);
    }
};

function updateFlie_HTML(oSourceHtml){
	fs.writeFile(__dirname+'/public/viewsource.html', oSourceHtml, function (err) {
	 if (err) throw err;
     console.log('Saved!');
	});
}